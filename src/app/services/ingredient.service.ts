  ...
  */
  getAllIngredients(): Observable<Ingredients[]> {
    ...
    */
    ...
    return this.http.post<Ingredients>(this.apiUrl, ingredient);
    }

    /**
    * Met à jour un ingrédient existant dans la base de données.
    * @param id - Identifiant de l'ingrédient à mettre à jour.
    * @param Ingredients - L'objet Ingredient mis à jour.
    * @returns Observable contenant l'ingrédient modifié.
    */
    updateIngredient(id: number, ingredient: Ingredients): Observable<Ingredients> {
    return this.http.put<Ingredients>(`${this.apiUrl}/${id}`, ingredient);
    }
